                    ...
                    <div className="flex items-center justify-between gap-2 w-full pr-2">
                      <div className="flex items-center gap-2 flex-wrap min-w-0">
                        {leave ? (
                          <>
                            {(() => {
                              // Same row, same data — a departure is only drawn
                              // differently because it is not a holiday.
                              const Icon = leave.kind === "departure" ? UserMinus : Plane;
                              return <Icon
                            ...
                            >
                              {leave.kind === "departure"
                                ? (leave.ongoing
                                    ? `Left ${Math.abs(leave.daysUntil)}d ago`
                                    : leave.daysUntil === 0 ? "Leaves today" : `Leaving in ${leave.daysUntil}d`)
                                : leave.ongoing ? "On leave now" : `Leave in ${leave.daysUntil}d`}
                            </Badge>
                            <span className="text-xs text-muted-foreground hidden sm:inline">
                              {leave.kind === "departure"
                                ? `Last day ${fmtDate(leave.startDate)}`
