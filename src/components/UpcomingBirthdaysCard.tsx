...
export function UpcomingBirthdaysCard() {
  ...
  const upcomingBirthdays = staffData.filter(staff => staff.date_of_birth).map(staff => {
  ...
  return <Card className="flex-1">
      ...
      <CardContent className="pt-0">
        {upcomingBirthdays.length === 0 ? <div className="text-center py-6 text-muted-foreground">
            <Cake className="h-8 w-8 mx-auto mb-2 opacity-50" />
            <p className="text-sm">No upcoming birthdays</p>
          </div> : <div className="space-y-3">
            {upcomingBirthdays.map(staff => <div key={staff.user_id} className="flex items-center justify-between p-3 rounded-lg bg-pink-50 dark:bg-pink-950/20">
                ...
                <div className="text-right">
                  {staff.daysUntil === 0 ? <span className="text-sm font-medium text-pink-600">Today! 🎂</span> : <span className="text-sm text-muted-foreground">
