...
function PageView({
  ...
  useEffect(() => {
    ...
    fetchPageSettings();
  }, [currentPage.id, currentPage.recommended_reading]);
...
export function KnowledgeBaseApp() {
  ...
  const fetchPageData = async (pageId: string): Promise<Page | null> => {
    try {
      console.log('Fetching fresh page data for:', pageId);
  ...
  const handleItemSelect = async (item: SidebarItem) => {
    ...
    if (item.id === 'home') {
      ...
      try {
        ...
        const pageData = await fetchPageData(item.id);
        ...
        if (pageData) {
          ...
          await supabase
            .from('pages')
            .update({ view_count: (await supabase.from('pages').select('view_count').eq('id', item.id).single()).data?.view_count || 0 + 1 })
            .eq('id', item.id);
        } else {
          toast({
            title: "Error loading page",
            description: "Page not found or failed to load.",
            variant: "destructive"
          });
        }
      } catch (error) {
  ...
  const handleSavePage = async (
    ...
    try {
      console.log('Saving page:', { id: currentPage.id, title, contentLength: content.length });
      ...
      if (currentPage.id === 'new') {
        ...
        console.log('New page created and state updated:', newPageData);
      } else {
        ...
        console.log('Page updated and state synced:', updatedPageData);
      }
  ...
  const handlePageSelect = async (pageId: string) => {
    ...
    await handleItemSelect({ id: pageId, title: '', type: 'page' });
  };

  // Handler to refresh current page data
  const refreshCurrentPage = async () => {
    if (currentPage && currentPage.id) {
      console.log('Refreshing current page data');
      const freshData = await fetchPageData(currentPage.id);
      if (freshData) {
        setCurrentPage(freshData);
        console.log('Current page refreshed with fresh data');
      }
    }
  };
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {currentView === 'editor' && currentPage && (
          <EnhancedContentEditor 
            ...
            onContentChange={refreshCurrentPage}
          />
        )}
        
        {currentView === 'page' && currentPage && !isLoadingPage && (
          <PageView 
