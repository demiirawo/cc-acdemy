...
export function PublicLiveView() {
  ...
  const { data: schedules = [], isLoading: schedulesLoading } = useQuery({
    queryKey: ["public-live-schedules", format(today, "yyyy-MM-dd")],
    queryFn: async () => {
  ...
  const { data: staffRequests = [] } = useQuery({
    queryKey: ["public-live-requests", format(today, "yyyy-MM-dd")],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("staff_requests")
        .select("id, user_id, request_type, swap_with_user_id, start_date, end_date, status, linked_holiday_id, coverage_metadata")
        .eq("status", "approved")
        .lte("start_date", format(today, "yyyy-MM-dd"))
        .gte("end_date", format(today, "yyyy-MM-dd"));
      if (error) throw error;
  ...
  const isStaffOnHoliday = (userId: string, date: Date): boolean => {
    return holidays.some(h => {
      ...
      return h.user_id === userId && isWithinInterval(date, { start, end });
    });
  };
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">
            Live View - {format(now, "EEEE, MMMM d, yyyy")} (Now: {format(now, "HH:mm")})
          </h1>
        </div>
      </div>
                ...
                return (
                  <div key={client} className="flex border-b last:border-b-0">
                    ...
                    >
                      ...
                      {/* Schedule bars */}
                      {Array.from(staffGroups.entries()).map(([userId, userSchedules], rowIndex) => (
                        <div 
                          key={userId} 
                          className="relative"
                          style={{ height: 50 }}
