...
export function UpcomingAnniversariesCard() {
  ...
  const { data: hrProfiles = [], isLoading: loadingHR } = useQuery({
  ...
  const upcomingAnniversaries = hrProfiles
    .filter(profile => profile.start_date)
    .map(profile => {
    ...
    .filter(profile => profile.nextAnniversary >= today && profile.nextAnniversary <= thirtyDaysFromNow && profile.yearsOfService > 0)
      ...
      <CardContent className="pt-0">
        {upcomingAnniversaries.length === 0 ? (
          ...
          <div className="space-y-3">
            {upcomingAnniversaries.map((profile) => (
              <div 
                key={profile.user_id} 
              ...
              >
                ...
                <div className="text-right">
                  {profile.daysUntil === 0 ? (
                    <span className="text-sm font-medium text-amber-600">Today! 🎉</span>
                  ) : (
